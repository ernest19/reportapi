...
from galamsey.models import RegionalBoundary,District
...
def index(request):
	...
	return render(request, 'charcoal/index.html', toHTML)
...
def send_notifications(request): 
	...
	for aa in reg_id:
		...
		message_title = "helloo EMAPPING TESTING"
		message_body = "JESUS IS LORD"
		...
		print reg_id.count()
		print aa.registration_id
	return HttpResponse("result")




def charcoalalertView(request):
	toHTML = {}
	print"hellooooo"
	...
	toHTML["alert"]=charcoalReports.objects.all().order_by("-created_date")

	print toHTML["alert"].count()

	# toHTML["alert"]=charcoalReports.objects.filter(created_date__date = dt.strftime("%Y-%m-%d")  )
	return render(request, 'charcoal/alert.html', toHTML)
...
# class charcoalReportspointView(GeoJSONLayerView):
...
def charcoalReportspointView(request):
	resulpoygon = []
	incident=charcoalReports.objects.all()
	for aa in incident:
...
def ExtentView(request):
	...
	tojson = charcoalReports.objects.filter(id=valuecode).aggregate(Extent('geom')).get('geom__extent')
...
# class charcoalReportsViewSet(viewsets.ModelViewSet):
#     queryset = charcoalReports.objects.all()
#     serializer_class = charcoalReportsSerializer
...
#     """
#     queryset = charcoalReports.objects.all().order_by('-created_date')
#     serializer_class = charcoalReportsSerializer


# @api_view(['GET', 'POST'])
# def charcoalReportsView(request):
#     """
#     List all code snippets, or create a new snippet.
#     """
#     if request.method == 'GET':
#         incident = charcoalReports.objects.all()
#         serializer = charcoalReportsSerializer(incident, many=True)
#         return Response(serializer.data)

#     elif request.method == 'POST':
#         serializer = charcoalReportsSerializer(data=request.data)
